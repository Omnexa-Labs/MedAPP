"""restore the created_at/updated_at server defaults on the hospital tables
...
`nullable=False` with **no** server_default, so every write dies on:
    ...
    "created_at" of relation "hospital_profiles" violates not-null constraint
...
TABLES = ('hospital_profiles', 'hospital_reviews', 'hospital_staff')
