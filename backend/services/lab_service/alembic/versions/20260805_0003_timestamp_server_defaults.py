"""restore the created_at/updated_at server defaults on the lab tables
...
`nullable=False` with **no** server_default, so every write dies on:
    ...
    "created_at" of relation "lab_orders" violates not-null constraint
...
TABLES = ('lab_orders', 'lab_results', 'partner_labs')
