...
- **`--localhost` is not optional.** `expo start` defaults to `--host lan`, which advertises
  `exp://<your-lan-ip>:8081` and encodes *that* into the QR code — an address the phone cannot
  reach, because this network isolates wireless clients. The symptom is Expo Go sitting on a
  loading screen **indefinitely, with no error at all**, which reads like a slow bundle rather
  than a network failure. `--localhost` advertises `exp://127.0.0.1:8081`, which the `adb reverse`
  tunnel forwards down the cable. Add `--clear` after any dependency change.
...
| **Expo Go loads forever, no error** | Almost always Metro's *bind address*, not your code. Check `Get-NetTCPConnection -LocalPort 8081 -State Listen`: if `LocalAddress` is `::1`, Metro is on IPv6 loopback only while `adb reverse` forwards to IPv4, so the phone talks to nothing. Want `::` (dual-stack). The script now uses `--host lan` + `REACT_NATIVE_PACKAGER_HOSTNAME=127.0.0.1` to get that. |
| A crashed Metro blocks the port | It can keep a `::1` listener — alive enough to block a rebind, dead enough to serve nothing, and `expo start` reports only `EADDRINUSE ::1:8081`. The script kills it now. |
