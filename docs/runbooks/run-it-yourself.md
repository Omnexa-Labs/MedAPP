...
| **Expo Go loads forever, no error** | Metro is on `--host lan` and the QR encodes `exp://<lan-ip>:8081`, which this network cannot deliver. Restart with `--localhost` so the QR encodes `127.0.0.1`. The script does this now. |
