  ...
  try {
    ...
    if (
      ...
      (typeof body.mode !== "string" ||
        !["medapp", "local"].includes(body.mode))
    )
      throw new HttpError(400, "Choose MedApp or local staff sign-in.");
    if (!verifying && body.mode === "local") {
      const { id, identity } = await sessions.beginLocal(
        payload.email,
        payload.password,
      );
      createdSession = id;
      ...
      response = await apiFetch(
        verifying ? "/v1/auth/two-factor/verify" : "/v1/auth/login",
        {
          ...
          headers: {
            ...
            "X-Device-Name": "MedApp Pharmacy web",
          },
    ...
    if (!response.ok) {
      if (response.status === 401)
        throw new HttpError(
          401,
          verifying
            ? "The code was not accepted or has expired. Try again or restart sign-in."
    ...
    if (locked && (await attempts.get(locked.id))?.scope !== body.scope) {
