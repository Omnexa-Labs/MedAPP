...
type Form = Record<keyof PharmacyDetails, string>;
const fields: {
  name: keyof PharmacyDetails;
  max?: number;
  multiline?: boolean;
  type?: string;
  hint?: string;
  ...
  { name: "description", max: 4000, multiline: true },
  {
    ...
    multiline: true,
    hint: "One service per line, up to 30. List only services currently offered.",
  },
  { name: "operating_hours" },
  {
    name: "photo_url",
  },
  { name: "head_pharmacist_name", max: 255 },
  { name: "head_pharmacist_bio", max: 1000, multiline: true },
  {
  ...
  { name: "phone", max: 64, type: "tel" },
  { name: "email", max: 255, type: "email" },
  {
    name: "website_url",
    max: 512,
    type: "url",
...
function toForm(details: PharmacyDetails): Form {
  return Object.fromEntries(
    fields.map(({ name }) => [
      name,
      name === "operating_hours"
        ? JSON.stringify(details.operating_hours || {})
...
function changesFrom(
  form: Form,
  saved: PharmacyDetails,
): Partial<PharmacyDetails> {
  ...
  return Object.fromEntries(
    fields
      ...
      .map(({ name }) => {
        ...
        if (name === "operating_hours") return [name, JSON.parse(text)];
        if (name === "insurance_accepted" || name === "services_offered")
          return [
            name,
            text
              .split("\n")
...
export function PharmacyProfileScreen() {
  ...
  const profile = useQuery({
    queryKey: ["pharmacy-profile", scope],
    queryFn: ({ signal }) => repository.get(signal),
    enabled: !!scope && role === "pharmacy_admin" && mode === "medapp",
    staleTime: Infinity,
    refetchOnWindowFocus: false,
  });
  if (role !== "pharmacy_admin" || !scope || mode !== "medapp")
    return (
      <p role="alert">
        Sign in with the approved MedApp pharmacy owner account to manage this
        profile.
      </p>
    );
  if (profile.isPending) return <p role="status">Loading pharmacy profile…</p>;
  if (profile.isError)
    return (
  ...
  return (
    <ProfileEditor
      key={`${scope}:${profile.data.pharmacy_id}`}
...
function ProfileEditor({
  ...
  const formDirty = fields.some(({ name }) => form[name] !== baseline[name]);
  const dirty = formDirty || !!photoFile;
  ...
  async function perform(
    action: "save" | "publish" | "withdraw" | "reload" | "uploadPhoto",
  ) {
    if (request.current) return;
    if (action === "uploadPhoto" && (!photoFile || formDirty)) return;
    const controller = new AbortController();
    request.current = controller;
    setBusy(true);
    setUploading(action === "uploadPhoto");
    setMessages([]);
    setNotice("");
    try {
      const result =
      ...
      if (controller.signal.aborted || useAuthStore.getState().scope !== scope)
        return;
      setSaved(result);
      if (action === "uploadPhoto" || action === "reload") setPhotoFile(null);
      if (action !== "withdraw" || !dirty) setForm(toForm(result.draft));
      setReloadRequired(false);
      setConfirmation(null);
      client.setQueryData(["pharmacy-profile", scope], result);
      void client.invalidateQueries({ queryKey: ["pharmacy-profile-history"] });
      setNotice(
        {
          save: "Draft saved. Publish it when you are ready for patients to see these details.",
          publish: "Pharmacy profile published to the patient directory.",
          withdraw: "Pharmacy listing withdrawn from the patient directory.",
          reload: "Latest saved profile loaded.",
          uploadPhoto:
            "Photo saved to your draft. Review the preview, then publish when you are ready.",
        }[action],
      );
    } catch (error) {
      if (controller.signal.aborted || useAuthStore.getState().scope !== scope)
    ...
    <div className="mx-auto max-w-7xl space-y-6">
      <header className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h1 className="text-3xl font-semibold">Pharmacy profile</h1>
          <p className="mt-2 max-w-2xl text-sm text-slate-600">
            Keep your pharmacy details up to date. Save a draft, review it, then
        ...
        <p className="font-medium">
          {saved.is_listed
            ? saved.has_unpublished_changes
              ...
              : "Patients can see your published pharmacy profile."
            : "This pharmacy is not listed in the patient directory."}
        </p>
        {dirty && (
      ...
      {uploading && (
        <p role="status">Uploading and preparing pharmacy photo…</p>
      )}
      {confirmation && (
        <section
          ...
          >
            {confirmation === "publish"
              ...
              : confirmation === "withdraw"
                ? "Withdraw this pharmacy listing?"
                : "Discard unsaved changes and reload?"}
          </h2>
          <p className="text-sm text-slate-600">
            {confirmation === "publish"
              ? "Review the saved draft preview below. These details will become visible in patient search and the pharmacy directory."
              : confirmation === "withdraw"
                ? "Patients will no longer find this pharmacy in the directory. Your saved draft is kept for future publication."
      ...
      <div className="grid items-start gap-6 xl:grid-cols-2">
        <form
          ...
          onSubmit={(event) => {
            event.preventDefault();
            if (formDirty && !busy && !reloadRequired && !confirmation)
              void perform("save");
          }}
        >
          <div>
            ...
            <p className="mt-1 text-sm text-slate-600">
              Only the pharmacy name is required to save. Publication also needs
              a complete address, contact details and a full weekly schedule.
            </p>
          </div>
          <fieldset disabled={busy || !!confirmation} className="space-y-4">
            <legend className="sr-only">Pharmacy directory details</legend>
            {fields.map(({ name, max, multiline, type, hint }) =>
              name === "photo_url" ? (
                <PhotoPicker
                  key={name}
                  file={photoFile}
                  hasPhoto={!!form.photo_url}
                  disabled={busy || reloadRequired || !!confirmation}
                  canUpload={!formDirty}
                  onSelect={setPhotoFile}
                  onUpload={() => void perform("uploadPhoto")}
                  onRemove={() => {
                    setPhotoFile(null);
                ...
                <div key={name} className="space-y-1.5">
                  ...
                  {name === "operating_hours" ? (
                    <HoursEditor
                      value={JSON.parse(form.operating_hours)}
                      onChange={(hours) =>
                        setForm((current) => ({
                          ...current,
          ...
          <Button
            type="submit"
            className="min-h-11 bg-teal-700 hover:bg-teal-800"
            disabled={!formDirty || busy || reloadRequired || !!confirmation}
          >
        ...
        <div className="space-y-5">
          ...
          >
            ...
            <p>License number: {saved.license_number || "Not provided"}</p>
            <p className="mt-1">
              License categories:{" "}
              {saved.license_categories.join(", ") || "Not provided"}
            </p>
            <p className="mt-3 text-slate-500">
              Contact support to correct approval details. The public pharmacy
