...
import { user, identity as baseIdentity } from "./session-fixture";
...
describe("pharmacy handoff screens", () => {
  ...
  it("keeps an explicit return link available after the scoped provider is replaced on sign-out", async () => {
    const url = `medapp://pharmacy-workspaces?handoff_state=${"s".repeat(32)}`;
    vi.stubGlobal(
      "fetch",
      vi.fn((path: string) =>
        Promise.resolve(
