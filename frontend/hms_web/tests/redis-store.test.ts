...
import { fixture, tokens, deviceId } from "./session-fixture";
vi.mock("server-only", () => ({}));
import { key, redis, sessionStore } from "../src/server/store";
const run = promisify(execFile);
const name = "medapp-hms-web-qa-" + randomBytes(6).toString("hex");
let started = false,
  connected = false;
describe.skipIf(process.env.HMS_WEB_TEST_REDIS !== "1")(
  ...
  () => {
    beforeAll(async () => {
      ...
      process.env.HMS_WEB_REDIS_URL = "redis://" + address;
      await redis().ping();
      connected = true;
    }, 90000);
    afterAll(async () => {
    ...
    async function record() {
      const f = fixture();
      const manager = new SessionManager(sessionStore, f.api);
      const login = await manager.begin(tokens, deviceId);
      return { ...login, session: (await sessionStore.get(login.id))! };
    }
