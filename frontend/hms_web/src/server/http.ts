...
export const cookieName = () =>
  process.env.NODE_ENV === "production" ? "__Host-medapp_hms" : "medapp_hms";
...
export function originCheck(request: Request) {
  const expected = process.env.HMS_WEB_ORIGIN;
  if (!expected)
    throw new HttpError(
      503,
      "The hospital service is temporarily unavailable.",
    );
  if (
    ...
    throw new HttpError(
      403,
      "This request must come from the hospital portal.",
    );
}
