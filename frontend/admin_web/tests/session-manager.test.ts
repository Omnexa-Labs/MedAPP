...
function fixture(refresh?: () => Promise<Response>) {
  ...
  const api = vi.fn(async (path: string, _init?: RequestInit) => {
    ...
    if (path === '/v1/auth/admin-sso/config')
      return Response.json({ enabled: true, hosted_domains: ['example.com'] });
    if (path === '/v1/me/providers')
      return Response.json({ items: [{ provider: 'google' }] });
...
describe('server-held sessions', () => {
  it('keeps the same device binding through refresh and logout', async () => {
    const f = fixture();
    const { id, session } = await f.manager.begin(tokens, 'd'.repeat(64));
    expect(session.expiresAt - 100000).toBe(8 * 3600000);
    f.expire();
    await f.manager.get(id);
    await f.manager.logout(id);
    for (const [path, init] of f.api.mock.calls.filter(([path]) =>
      /refresh|logout/.test(path),
    )) {
      expect(new Headers(init?.headers).get('X-Device-Id'), path).toBe(
        'd'.repeat(64),
      );
    }
  });
  it('rejects ordinary accounts even when they have a connected Google identity', async () => {
    const f = fixture();
    f.api.mockImplementationOnce(async () =>
      Response.json({ ...user, role: 'patient' }),
    );
    await expect(f.manager.begin(tokens, 'd'.repeat(64))).rejects.toMatchObject(
      { status: 403 },
    );
    expect(f.store.values.size).toBe(0);
  });
  it('closes the session immediately when the current role loses administrator access', async () => {
    const f = fixture();
    const { id } = await f.manager.begin(tokens, 'd'.repeat(64));
    f.api.mockImplementationOnce(async () =>
      Response.json({ ...user, role: 'patient' }),
    );
  ...
  it('closes the session when Google is disconnected', async () => {
    const f = fixture();
    const { id } = await f.manager.begin(tokens, 'd'.repeat(64));
    f.api
      .mockImplementationOnce(async () => Response.json(user))
      .mockImplementationOnce(async () =>
        Response.json({ enabled: true, hosted_domains: ['example.com'] }),
      )
  ...
  it('returns public identity without authentication tokens', async () => {
    const f = fixture();
    const login = await f.manager.begin(tokens, 'd'.repeat(64));
